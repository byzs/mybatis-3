...
public class Configuration {
  ...
  /**
   * 从 SQL 中移除额外的空格字符。请注意，这也会影响 SQL 中的文字字符串。（自 3.5.5 起）
   */
  protected boolean shrinkWhitespacesInSql;

  /**
   * 指定“foreach”标签上“nullable”属性的默认值。（自 3.5.9 起）
   */
  protected boolean nullableOnForEach;

  /**
   * 应用构造函数自动映射时，使用参数名称搜索要映射的列，而不是依赖于列顺序。（自 3.5.10 起）
   */
  ...
  /**
   * 指定一个包含提供程序方法的 SQL 提供程序类（自 3.5.6 起）。
   * 此类适用于 SQL 提供程序注释上的 type（或 value）属性（例如 @SelectProvider），当这些属性被省略时。
   */
  ...
  /**
   * 指定语句默认的滚动策略。[ORWARD_ONLY | SCROLL_SENSITIVE | SCROLL_INSENSITIVE | DEFAULT（等同于未设置）]
   */
  ...
  /**
   * 指定 Mybatis 创建可延迟加载对象所用到的代理工具。 [CGLIB | JAVASSIST] mybatis3.3以上默认为 javassist
   */
